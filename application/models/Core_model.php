...
class Core_model extends CI_Model
{
	...
	public function insert($tabela = null, $data = null, $get_last_id = null)
	{
		if ($tabela && $this->db->table_exists($tabela) && is_array($data)) {
			...
			/*
			 * Se get_last_id veio como parâmetro, então será inserido na sessão o último ID criado no banco de dodo da $tabela
			 */
			if ($get_last_id) {
	...
	public function delete($tabela = null, $condicoes = null)
	{
		if ($tabela && $this->db->table_exists($tabela) && is_array($condicoes)) {
			...
			if ($this->db->delete($tabela, $condicoes)) {
				$this->session->set_flashdata('sucesso', 'Dados excluído com sucesso');
			} else {
				$this->session->set_flashdata('Erro', 'Não foi possível excluído o dado');
			}
		} else {
			return false;
		}
